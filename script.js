...
document.addEventListener("DOMContentLoaded", () => {
  ...
  reportForm?.addEventListener("submit", (ev) => {
    ...
    function saveReport(imgData) {
      ...
      const _s = JSON.parse(localStorage.getItem(LS_SESSION) || "null");
      arr.push({ id, title, location, desc, type, image: imgData || "", createdAt: Date.now(), claimed: false, status: "active", claimRequests: [], reporterId: _s ? _s.id : "" });
  ...
  function renderCounts() {
    ...
    animNum("countLost",    reports.filter(r => r.type === "lost" && !r.claimed && r.status !== "archived").length);
    animNum("countFound",   reports.filter(r => r.type === "found" && r.status !== "archived").length);
    animNum("countClaimed", reports.filter(r => r.claimed || r.status === "claimed").length);
  }
  ...
  function renderRecent() {
    ...
    list.forEach(r => {
      ...
      item.innerHTML = `
        ...
        <div class="badge ${getStatusBadgeClass(r)}">${getStatusLabel(r)}</div>
      `;
  ...
  function runSearch() {
    ...
    const results = reports.filter(r => {
      ...
      else if (filt === "claimed") okType = r.claimed || r.status === "claimed";
  ...
  window.showItemDetail = (itemId) => {
    ...
    const pinned = isPinned(r.id);
    const curUser = (() => { try { return JSON.parse(localStorage.getItem(LS_SESSION) || "null"); } catch(e) { return null; } })();
    const isOwner = curUser && r.reporterId && curUser.id === r.reporterId;
    const alreadyRequested = curUser && (r.claimRequests || []).some(req => req.requesterId === curUser.id);
    const pendingRequests = (r.claimRequests || []).filter(req => req.status === "pending");

    // Status pipeline HTML
    const pipeline = buildStatusPipeline(r);

    // Claim request button section
    let claimSection = "";
    const isArchived = r.status === "archived";
    const isClaimed  = r.claimed || r.status === "claimed";

    if (!isClaimed && !isArchived) {
      if (isOwner) {
        // Owner sees pending claim requests
        if (pendingRequests.length > 0) {
          claimSection = `
            <div class="claim-requests-box">
              <div class="claim-box-title">📬 Claim Requests (${pendingRequests.length})</div>
              ${pendingRequests.map((req, idx) => {
                const realIdx = (r.claimRequests || []).indexOf(req);
                return `<div class="claim-request-row">
                  <div class="claim-req-info">
                    <strong>${escapeHtml(req.requesterName)}</strong>
                    <span class="claim-req-msg">${escapeHtml(req.message || "No message")}</span>
                    <span class="claim-req-time">${nowStr(req.timestamp)}</span>
                  </div>
                  <div class="claim-req-btns">
                    <button class="approve-btn" onclick="approveClaim('${r.id}', ${realIdx})">✅ Approve</button>
                    <button class="reject-btn" onclick="rejectClaim('${r.id}', ${realIdx})">❌ Reject</button>
                  </div>
                </div>`;
              }).join("")}
            </div>`;
        } else {
          claimSection = `<div class="claim-info-box">📭 No pending claim requests yet.</div>`;
        }
      } else if (!curUser) {
        claimSection = `<button class="primary-btn" onclick="showGuestModal()">🔑 Login to Claim</button>`;
      } else if (alreadyRequested) {
        claimSection = `<div class="claim-info-box pending-info">⏳ Your claim request is pending review.</div>`;
      } else if (r.status !== "claim_requested" && r.status !== "claim_approved") {
        claimSection = `<button class="primary-btn" onclick="openClaimModal('${r.id}')">📨 Request to Claim</button>`;
      }
    }
    ...
    document.getElementById("detailContent").innerHTML = `
      ...
      <div class="detail-body">
        ...
        <div class="detail-meta">
          ...
          <span class="badge ${getStatusBadgeClass(r)}" style="margin-left:4px">${getStatusLabel(r)}</span>
        </div>
        ${pipeline}
        ...
        <div class="detail-actions">
          ...
          ${isOwner || !r.reporterId ? `<button class="delete-btn" style="width:auto" onclick="deleteItem('${r.id}')">🗑️ Delete</button>` : ""}
        </div>
      </div>
    `;
  };
  ...
  window.claimItem = (itemId) => { window.openClaimModal(itemId); }; // legacy redirect
  ...
  window.openClaimModal = (itemId) => {
    ...
    modal.id = "claimModal";
    modal.innerHTML = `
      <div class="claim-modal-overlay" onclick="document.getElementById('claimModal').remove()">
        <div class="claim-modal-box" onclick="event.stopPropagation()">
          <div class="claim-modal-title">📨 Request to Claim</div>
          <div class="claim-modal-item">"${escapeHtml(item.title)}"</div>
          <textarea id="claimMsgInput" class="claim-msg-input" placeholder="Add a message to the owner (e.g. description of the item to verify ownership)..." rows="3"></textarea>
          <div class="claim-modal-actions">
            <button class="primary-btn" onclick="submitClaimRequest('${itemId}')">Send Request</button>
            <button class="secondary-btn" onclick="document.getElementById('claimModal').remove()">Cancel</button>
          </div>
        </div>
      </div>`;
    document.body.appendChild(modal);
    document.getElementById("claimMsgInput").focus();
  };

  window.submitClaimRequest = (itemId) => {
    if (!requireLogin(() => {})) return;
    const msg = (document.getElementById("claimMsgInput")?.value || "").trim();
    const curUser = JSON.parse(localStorage.getItem(LS_SESSION) || "null");
    if (!curUser) return;
    const reports = lsRead(LS_REPORTS);
    const item = reports.find(r => r.id === itemId);
    if (!item) return;
    if (!item.claimRequests) item.claimRequests = [];
    if (item.claimRequests.some(req => req.requesterId === curUser.id)) {
      showToast("⚠️ You already submitted a request for this item.", "error");
      document.getElementById("claimModal")?.remove();
      return;
    }
    item.claimRequests.push({ requesterId: curUser.id, requesterName: curUser.name || curUser.email || "User", message: msg, timestamp: Date.now(), status: "pending" });
    item.status = "claim_requested";
    lsWrite(LS_REPORTS, reports);
    document.getElementById("claimModal")?.remove();
    showToast("📨 Claim request sent! Waiting for owner's approval.", "success");
    renderCounts(); renderRecent(); runSearch(); renderNotifications();
    showItemDetail(itemId);
  };

  window.approveClaim = (itemId, reqIdx) => {
    if (!requireLogin(() => {})) return;
    const reports = lsRead(LS_REPORTS);
    const item = reports.find(r => r.id === itemId);
    if (!item || !item.claimRequests[reqIdx]) return;
    item.claimRequests[reqIdx].status = "approved";
    item.claimRequests.forEach((req, i) => { if (i !== reqIdx && req.status === "pending") req.status = "rejected"; });
    item.status = "claimed";
    item.claimed = true;
    lsWrite(LS_REPORTS, reports);
  ...
  window.rejectClaim = (itemId, reqIdx) => {
    if (!requireLogin(() => {})) return;
    const reports = lsRead(LS_REPORTS);
    const item = reports.find(r => r.id === itemId);
    if (!item || !item.claimRequests[reqIdx]) return;
    item.claimRequests[reqIdx].status = "rejected";
    const stillPending = item.claimRequests.some(req => req.status === "pending");
    if (!stillPending) item.status = "active";
    lsWrite(LS_REPORTS, reports);
  ...
  window.filterByCategory = (category) => {
    ...
    const reports = lsRead(LS_REPORTS);
    const results = reports.filter(r => {
      if (r.type !== "lost" || r.claimed || r.status === "archived") return false;
...
// ═══════════════════════════════════════════════════════════════
// FINDTRACK v4 — CLAIM WORKFLOW · AUTO-ARCHIVE · STATUS TRACKING
// ═══════════════════════════════════════════════════════════════

// ── STATUS HELPERS ─────────────────────────────────────────────
function getStatusLabel(r) {
  if (r.status === "archived")         return "ARCHIVED";
  if (r.status === "claimed" || r.claimed) return "CLAIMED";
  if (r.status === "claim_approved")   return "APPROVED";
  if (r.status === "claim_requested")  return "PENDING CLAIM";
  if (r.type === "found")              return "FOUND";
  return "LOST";
}
...
function getStatusBadgeClass(r) {
  ...
  if (r.status === "claim_approved")   return "approved";
  if (r.status === "claim_requested")  return "pending-claim";
  return r.type; // "lost" or "found"
}

// ── STATUS PIPELINE ────────────────────────────────────────────
function buildStatusPipeline(r) {
  const steps = [
    { key: "active",           label: "Reported",       icon: "📦" },
    { key: "claim_requested",  label: "Claim Requested",icon: "📨" },
    { key: "claim_approved",   label: "Approved",       icon: "✅" },
    { key: "claimed",          label: "Claimed",        icon: "🎉" },
  ];
  const order = ["active","claim_requested","claim_approved","claimed"];
  let curIdx = order.indexOf(r.status);
  if (r.claimed) curIdx = 3;
  if (r.status === "archived") {
    return `<div class="status-pipeline"><div class="pipeline-archived">📦 This item has been auto-archived (30+ days old)</div></div>`;
  }
  const html = steps.map((s, i) => {
    let cls = "pipeline-step";
    if (i < curIdx) cls += " done";
    else if (i === curIdx) cls += " active";
    return `<div class="${cls}"><span class="pipeline-icon">${s.icon}</span><span class="pipeline-label">${s.label}</span></div>`;
  }).join('<div class="pipeline-line"></div>');
  return `<div class="status-pipeline">${html}</div>`;
}

// ── AUTO-ARCHIVE ────────────────────────────────────────────────
function autoArchiveItems() {
  ...
  try {
    const reports = lsRead(LS_REPORTS);
    reports.forEach(r => {
      if (r.status !== "claimed" && r.status !== "archived" && !r.claimed) {
    ...
    if (changed) lsWrite(LS_REPORTS, reports);
  } catch(e) {}
}
...
function renderNotifications() {
  ...
  reports.forEach(r => {
    if (!r.claimRequests || r.claimRequests.length === 0) return;
    const isOwner = r.reporterId && curUser.id === r.reporterId;

    r.claimRequests.forEach((req, idx) => {
      // Owner: see incoming requests
      if (isOwner && req.status === "pending") {
        notifications.push({
          type: "incoming",
          icon: "📬",
          title: `New claim request on "${r.title}"`,
          msg: `From <strong>${escapeHtml(req.requesterName)}</strong>: "${escapeHtml(req.message || "No message")}"`,
          time: req.timestamp,
          action: `window.showItemDetail('${r.id}')`,
          label: "Review"
        });
      }
      // Requester: see their request status
      if (req.requesterId === curUser.id && req.status !== "pending") {
        notifications.push({
          type: req.status === "approved" ? "approved" : "rejected",
          icon: req.status === "approved" ? "✅" : "❌",
          title: req.status === "approved" ? `Your claim on "${r.title}" was approved!` : `Your claim on "${r.title}" was rejected.`,
          msg: req.status === "approved" ? "Congratulations — the owner approved your claim." : "The owner declined this claim request.",
          time: req.timestamp,
          action: `window.showItemDetail('${r.id}')`,
          label: "View Item"
        });
  ...
  // Archived items notification
  const archived = reports.filter(r => r.status === "archived" && r.reporterId === curUser.id);
  if (archived.length > 0) {
    notifications.push({
      type: "archive",
      icon: "📦",
      title: `${archived.length} item(s) auto-archived`,
      msg: `Items older than 30 days are automatically archived to keep the feed clean.`,
      time: Date.now(),
      action: null,
      label: null
    });
  ...
  if (notifications.length === 0) {
    alertsList.innerHTML = `
      <div class="alert-item"><strong>🎉 Welcome to FindTrack!</strong><p>You'll receive notifications here for claim requests and status updates.</p></div>
      <div class="alert-item"><strong>💡 Pro Tip</strong><p>Report items with photos and detailed descriptions for the best chance of recovery!</p></div>`;
    return;
  }

  notifications.sort((a, b) => b.time - a.time);
  alertsList.innerHTML = notifications.map(n => `
    <div class="alert-item notif-${n.type}">
      <div class="notif-header"><span class="notif-icon">${n.icon}</span><strong>${n.title}</strong></div>
      <p>${n.msg}</p>
      ${n.action ? `<button class="notif-btn" onclick="${n.action}">${n.label}</button>` : ""}
    </div>`).join("");
}
window.renderNotifications = renderNotifications;

// Re-render notifications when notifications panel is opened
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll("[data-tab='notifications'], [data-section='notifications']").forEach(btn => {
    btn.addEventListener("click", () => setTimeout(renderNotifications, 100));
  });
});
